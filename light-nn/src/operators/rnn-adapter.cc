...
RNNAdapter::RNNAdapter(const Json::Value &config) : Operator(config) {
...
RNNAdapter::~RNNAdapter() {
}

bool RNNAdapter::set_weight(const std::vector<Tensor> &weights,
                            const std::map<std::string, size_t> &weights_name2id) {
  return true;
}

bool RNNAdapter::reshape(const std::vector<Tensor *> &input,
...
void RNNAdapter::forward_impl(const std::vector<Tensor *> &input,
                              std::vector<Tensor *> &output) {
