// Copyright (c) 2017, Tencent Inc.
// All Rights Reserved
//
// Author: Wenfeng Xuan <johnxuan@tencent.com>
//
#ifndef LNN_OPERATORS_SCALE_H_
#define LNN_OPERATORS_SCALE_H_

#include "operator.h"

namespace lnn {

class Scale : public Operator {
 public:
  Scale(const Json::Value &config);
  virtual ~Scale();

  virtual inline const char * type() const { return "Scale"; }

  virtual bool set_weight(const std::vector<Tensor> &weights,
                          const std::map<std::string, size_t> &weights_name2id);

  virtual bool reshape(const std::vector<Tensor *> &input,
                       std::vector<Tensor *> &output);

 private:
  virtual void forward_impl(const std::vector<Tensor *> &input,
                            std::vector<Tensor *> &output);

  bool b_set;  // whether the scale is set in json file
  float m_scale;
};

}  // namespace lnn

#endif  // LNN_OPERATORS_SCALE_H_
